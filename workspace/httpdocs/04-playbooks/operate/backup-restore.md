...
This operations playbook covers backup restore for production use.
...
export async function backup_restore_handler() {
  // Implementation
}
```
...
```typescript
try {
  const result = await backup_restore_handler();
  return result;
} catch (error) {
