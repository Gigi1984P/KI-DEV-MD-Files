...
This operations playbook covers incident response for production use.
...
export async function incident_response_handler() {
  // Implementation
}
```
...
```typescript
try {
  const result = await incident_response_handler();
  return result;
} catch (error) {
